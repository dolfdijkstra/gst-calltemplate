...
public class CallTemplateWithStyle extends CallTemplateWithContextSetting
{
	...
	/* (non-Javadoc)
	 * @see com.openmarket.xcelerate.jsp.render.CallTemplateWithContextSetting#doEndTag(COM.FutureTense.Interfaces.ICS, boolean)
	 */
	@Override
	...
	private String fixStyle(ICS ics)
	{
		...
		/**
		 * Considerations
		 * 1) Check target for parameter callstyle and use that 
		 * 
		 */
		String pname;
		...
		//String targetStyle =(String) ics.getPageData(pname).getDefaultArguments().get("callstyle");
		boolean targetCached = this.isCacheable(ics, pname);
		boolean currentCached = this.isCacheable(ics, ics.GetVar(ftMessage.PageName));

		String proposal = proposeStyle(ics, pname, currentCached, targetCached);
	...
	private String proposeStyle(ICS ics, String pname, boolean currentCache, boolean targetCache)
	{
		if (currentCache == false) //we are not caching for the current pagelet
		{
			...
			{
				return "element"; //call as element is target is also not cacheable
			}
			else
			{
		...
		{ //currently we are caching
			...
			{
				//LOG.debug("getvar.cid=" + ics.GetVar("cid") + " at " + ics.pageURL());
				...
				//should we check if cid is current page criteria, we are a Template??
				...
				{
					checkPageCriteria(ics, pname);
					return "embedded"; // this is calltemplate, assuming that headers/footers/leftnavs etc will be via CSElements/SiteEntry
				}
			}
	...
	/**
	 * Checks if the pagelet should be cached. Takes into consideration if current pagelet is rendered for Satellite Server. 
	 * 
	 * @param ics
	...
	/* (non-Javadoc)
	 * @see com.openmarket.xcelerate.jsp.render.CallTemplate#setC(java.lang.String)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see com.openmarket.xcelerate.jsp.render.CallTemplate#setSite(java.lang.String)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see com.openmarket.xcelerate.jsp.render.CallTemplate#setTname(java.lang.String)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see com.openmarket.xcelerate.jsp.render.CallTemplate#setCid(java.lang.String)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see com.openmarket.xcelerate.jsp.render.CallTemplate#setStyle(java.lang.String)
	 */
	@Override
